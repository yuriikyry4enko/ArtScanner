...
[assembly: Xamarin.Forms.Dependency(typeof(ArtScanner.iOS.Services.DownloadFileService))]
namespace ArtScanner.iOS.Services
{
    public class DownloadFileService : IDownloadFileService
    {
		...
		public async Task DownloadFile(string url, string fileName, Stream responseStream)
        {
			...
			try
			{
				inputStream = GetQueueStream(responseStream, fileName);
				...
				//	{
				//		l += inputStreamLength;
				//		//TODO:Check with true option 
				//		player.ParseBytes(buffer, inputStreamLength, false, false);
		...
		Stream GetQueueStream(Stream responseStream, string fileName)
		{
			var queueStream = new QueueStream(Environment.GetFolderPath(Environment.SpecialFolder.Personal) + $"/{fileName}");
			var t = new Thread((x) => {
